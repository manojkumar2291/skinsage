...
def test_forgot_password(mock_db_connection):
    ...
    with patch("app.api.auth.send_email_sync") as mock_send_email:
        with patch("app.api.auth.get_db_connection", return_value=mock_connection):
            ...
            assert response.status_code == 200
            assert "reset link" in response.json()["msg"]
            mock_cursor.execute.assert_called()
            mock_connection.commit.assert_called_once()

def test_reset_password(mock_db_connection):
    ...
    with patch("app.api.auth.get_db_connection", return_value=mock_connection):
        ...
        # Verify 3 executes: 1. check token, 2. update pass, 3. mark used
